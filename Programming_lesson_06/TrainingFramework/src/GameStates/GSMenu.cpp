...
void GSMenu::Init()
{
	...
	button->Set2DPosition(screenWidth / 4, screenHeight - 80);
	...
	button = std::make_shared<GameButton>(model, shader, texture);
	button->Set2DPosition(2 * screenWidth / 4, screenHeight - 80);
	button->SetSize(227, 63);
	button->SetOnClick([]() {
	...
	button = std::make_shared<GameButton>(model, shader, texture);
	button->Set2DPosition(3 * screenWidth / 4, screenHeight - 80);
	button->SetSize(227, 63);
	button->SetOnClick([]() {
		exit(0);
		});
...
//	m_Text_gameName = std::make_shared< Text>(shader, font, "MONKEY MEYKON", TEXT_COLOR::GREEN, 1.0);
