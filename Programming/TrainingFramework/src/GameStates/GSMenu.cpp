...
void GSMenu::Init()
{
	...
	//play button
	texture = ResourceManagers::GetInstance()->GetTexture("icon_play");
	std::shared_ptr<GameButton> button = std::make_shared<GameButton>(model, shader, texture);
	button->Set2DPosition(Application::screenWidth / 2, 250);
	button->SetSize(200, 200);
	button->SetOnClick([]() {
		SoundManager::GetInstance()->PauseSound("menu_bground");
	...
	button->Set2DPosition(Application::screenWidth / 4, Application::screenHeight - 80);
	...
	button = std::make_shared<GameButton>(model, shader, texture);
	button->Set2DPosition(3 * Application::screenWidth / 4, Application::screenHeight - 80);
	button->SetSize(227, 63);
	button->SetOnClick([]() {
		exit(0);
		});
	...
	SoundManager::GetInstance()->AddSound("menu_bground");

	SoundManager::GetInstance()->PlaySound("menu_bground");
